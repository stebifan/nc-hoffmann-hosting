...
if [[ $(find var/www/html/config/config.php -type f -size -100c 2>/dev/null) ]]; then
    ...
    su - www-data -s /bin/sh -c "php /var/www/html/occ maintenance:install -q -n --database-host "db" --database "pgsql" --database-name "nextcloud"  --database-user "nextcloud" --database-pass "$POSTGRES_PASSWORD" --admin-user "$NEXTCLOUD_ADMIN_USER" --admin-pass "$NEXTCLOUD_ADMIN_PASSWORD" --data-dir "/var/www/html/data""
fi
